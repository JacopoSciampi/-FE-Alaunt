<?php
	...
	if((empty($workers))||(empty($username))){
		...
		if($row_cnt >= 1){
			...
			$r1 = $con->query("SELECT * FROM b_iron WHERE level = '$lvIron'");
			...
			if(($mineWood >= $woodCost) && ($mineStone >= $stoneCost)){
				...
				$updatingUP = "UPDATE updating SET name='$name', tstart='$timeStart', tend='$timeEnd', nworkers='$workers', points='$points', newlevel='$lvIron', ttotal = '$diffTime' WHERE userID='$userID'";
