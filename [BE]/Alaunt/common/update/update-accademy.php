<?php
	...
	if((empty($workers))||(empty($username))){
		...
		if($row_cnt >= 1){
			...
			$lvAccademy = $BUILDLEVEL[9];
			$lvAccademyMore = $lvAccademy + 1;
			
			// Get spec for next level
			$r1 = $con->query("SELECT * FROM accademy WHERE level = '$lvAccademyMore'");
			...
			if(($mineWood >= $woodCost) && ($mineStone >= $stoneCost)&& ($mineIron >= $ironCost)&& ($mineOre >= $oreCost) ){
				...
				$updatingUP = "UPDATE updating SET name='$name', tstart='$timeStart', tend='$timeEnd', nworkers='$workers', points='$points', newlevel='$lvAccademyMore', ttotal = '$diffTime' WHERE userID='$userID'";
