	...
	if((empty($workers))||(empty($username))){
		...
		if($row_cnt >= 1){
			...
			if(($mineWood >= $woodCost) && ($mineStone >= $stoneCost)){
				...
				$updatingUP = "UPDATE updating SET name='$name', tstart='$timeStart', tend='$timeEnd', nworkers='$workers', points='$points', newlevel='$lvFood', ttotal = '$diffTime' WHERE userID='$userID'";
